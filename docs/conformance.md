...
| `PLAY "<MML>"` | **Microsoft BASIC** (GW-BASIC / QuickBASIC) | Play a Music Macro Language string. Commands: notes `A`–`G` with `#`/`+`/`-` accidentals and an optional length digit and dots; `O n` octave (0–6, default 4); `>`/`<` octave shift; `N n` note number (0–84, 0 = rest); `L n` default length (1–64, default 4); `P n` rest; `T n` tempo (32–255, default 120); `MN`/`ML`/`MS` normal/legato/staccato (note sounds 7/8 / full / 3/4 of its slot); `MF`/`MB` foreground/background. **Implementation-defined / deviations:** pitch is equal-tempered with A4 = 440 Hz (octave numbers follow scientific pitch notation; GW-BASIC's 8253-divisor frequencies differ by a few cents); `MB` background music is parsed but rendered as foreground (no async yet); `X` substring and `=` variable substitution are parsed-and-rejected (not yet implemented). Output backends: a deterministic `--wav` file (square wave, 44.1 kHz/16-bit mono) on `run`/`vm`; real-time audible playback is a later phase. The full sourced spec and phase plan are in [audio-extension.md](audio-extension.md). |
