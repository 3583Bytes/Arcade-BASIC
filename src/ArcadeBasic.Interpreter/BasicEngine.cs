...
public static class BasicEngine
{
    ...
    public static Result Run(string source, TextWriter stdout, TextReader? stdin = null, string filename = "<embedded>", CancellationToken cancel = default)
    {
        ...
        var interp = new ArcadeBasic.Interpreter.BasicInterpreter(program, info, stdout, stdin ?? TextReader.Null, cancel);
        int exitCode;
        try
        {
            exitCode = interp.Run();
