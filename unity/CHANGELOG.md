...
### Added
- Audio output (`SOUND` / `BEEP` / `PLAY`). A `BasicAudioOutput` component (`Runtime/UnityAudio`) streams a program's audio through a looping `AudioClip` whose PCM-reader callback drains the engine-agnostic `BufferedAudioDevice` on Unity's audio thread — the audio analogue of `BasicScreen`. The **Arcade BASIC IDE** sample plays audio automatically (fresh device per Run, stopped on Stop/finish). Foreground (`MF`) and background (`MB`) music both work.
- Bundled examples now mirror the repo's full `examples/` library (all programs the CLI and TUI IDE ship, including the real-time games) instead of a hand-picked subset, kept in sync by `unity/scripts/copy-examples.sh`. The Open dropdown still groups them by `@category`; the Unity-only `keys.bas` input demo is preserved.
...
- `Arcade BASIC IDE` sample: a drop-in in-game BASIC IDE shipped as a ready-to-play **scene + prefab** (the `ArcadeBasicCodeEditor` MonoBehaviour with its UI wired in the Inspector).
  ...
  - **Build standalone** (Editor-only): pops a **target-platform menu** (Windows / macOS Apple Silicon / macOS Intel / Linux) and appends the source's compiled bytecode to that platform's `arcade-basic` AOT stub — so you can build for any OS from one machine (the bytecode is OS-agnostic; only the stub differs). Platforms are enabled per available stub; the host adds `buildStubPath` / `PATH` / file-picker fallbacks. Per-RID stubs ship under the sample's `Stubs/` folder in the release package. (Cross-OS output built on Windows needs `chmod +x` on the target; unsigned macOS binaries need a Gatekeeper bypass.)
