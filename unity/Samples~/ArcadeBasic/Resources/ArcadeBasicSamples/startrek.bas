REM SUPER STARTREK - MAY 16, 1978 - REQUIRES 24K MEMORY  @category Games
REM **** STAR TREK ****
...
REM **** MODIFICATIONS BY BOB LEEDOM - APR & DEC 1974.
REM **** CONVERTED TO MICROSOFT 8K BASIC 3/16/78 BY JOHN GORDERS.
REM **** PORTED TO FULL BASIC (ISO/IEC 10279) - 2026.
REM
...
OPTION BASE 1
RANDOMIZE
REM Pre-declare names whose first lexical reference is a read inside a
REM subroutine target or DEF FN body. Without these the analyzer warns
REM FB0308 (implicit declaration); behaviour is unchanged either way.
LET G2$ = ""
LET I = 0
LET S1 = 0
LET S2 = 0
...
227 PRINT
    ...
    PRINT
270 LET Z$="                         "
330 DIM G(8,8), C(9,2), K(3,3), NB(3), Z(8,8), D(8)
370 LET T = INT(RND(1)*20+20)*100
    LET T0 = T
    LET T9 = 25 + INT(RND(1)*10)
    LET D0 = 0
    LET E = 3000
    LET E0 = E
440 LET P = 10
...
670 FOR I = 1 TO 8
      LET D(I) = 0
    NEXT I
710 LET A1$ = "NAVSRSLRSPHATORSHEDAMCOMXXX"
...
820 FOR I = 1 TO 8
      FOR J = 1 TO 8
        ...
        LET Z(I,J) = 0
        LET R1 = RND(1)
850     IF R1 > 0.98 THEN LET K3 = 3 : LET K9 = K9 + 3 : GOTO 980
860     IF R1 > 0.95 THEN LET K3 = 2 : LET K9 = K9 + 2 : GOTO 980
870     IF R1 > 0.80 THEN LET K3 = 1 : LET K9 = K9 + 1
980     LET B3 = 0
        IF RND(1) > 0.96 THEN LET B3 = 1 : LET B9 = B9 + 1
1040    LET G(I,J) = K3*100 + B3*10 + FNR(1)
      NEXT J
    NEXT I
    IF K9 > T9 THEN LET T9 = K9 + 1
1100 IF B9 <> 0 THEN GOTO 1200
1150 IF G(Q1,Q2) < 200 THEN LET G(Q1,Q2) = G(Q1,Q2) + 120 : LET K9 = K9 + 1
1160 LET B9 = 1
     LET G(Q1,Q2) = G(Q1,Q2) + 10
     LET Q1 = FNR(1)
     LET Q2 = FNR(1)
1200 LET K7 = K9
     IF B9 <> 1 THEN LET X$ = "S" : LET X0$ = " ARE "
...
1430 GOSUB 9030
     PRINT
     IF T0 <> T THEN GOTO 1490
1460 PRINT "YOUR MISSION BEGINS WITH YOUR STARSHIP LOCATED"
1470 PRINT "IN THE GALACTIC QUADRANT, '"; G2$; "'."
     GOTO 1500
1490 PRINT "NOW ENTERING "; G2$; " QUADRANT . . ."
1500 PRINT
     LET K3 = INT(G(Q1,Q2) * 0.01)
     LET B3 = INT(G(Q1,Q2) * 0.1) - 10*K3
1540 LET S3 = G(Q1,Q2) - 100*K3 - 10*B3
     IF K3 = 0 THEN GOTO 1590
1560 PRINT "COMBAT AREA      CONDITION RED"
     IF S > 200 THEN GOTO 1590
...
1600 FOR I = 1 TO 3
       LET K(I,3) = 0
     NEXT I
     LET Q$ = Z$ & Z$ & Z$ & Z$ & Z$ & Z$ & Z$ & LEFT$(Z$, 17)
1660 REM POSITION ENTERPRISE, THEN PLACE "K3" KLINGONS, "B3" BASES, "S3" STARS.
1680 LET A$ = "<*>"
     LET Z1 = S1
     LET Z2 = S2
     GOSUB 8670
     IF K3 < 1 THEN GOTO 1820
1720 FOR I = 1 TO K3
       GOSUB 8590
       LET A$ = "+K+"
       LET Z1 = R1
...
2470 PRINT "WARP ENGINES ARE DAMAGED.  MAXIMUM SPEED = WARP 0.2"
     GOTO 1990
2490 LET N = INT(W1*8 + 0.5)
     IF E - N >= 0 THEN GOTO 2590
...
2590 FOR I = 1 TO K3
       ...
       LET A$ = "+K+"
       GOSUB 8670
2695   REM end of klingon-move loop
     NEXT I
2700 GOSUB 6000
     LET D1 = 0
     LET D6 = W1
     IF W1 >= 1 THEN LET D6 = 1
2770 FOR I = 1 TO 8
       IF D(I) >= 0 THEN GOTO 2885
2790   LET D(I) = D(I) + D6
       IF D(I) > -0.1 AND D(I) < 0 THEN LET D(I) = -0.1 : GOTO 2885
2800   IF D(I) < 0 THEN GOTO 2885
2810   IF D1 <> 1 THEN LET D1 = 1 : PRINT "DAMAGE CONTROL REPORT:  ";
2840   PRINT TAB(8);
       LET R1 = I
       GOSUB 8790
       PRINT G2$; " REPAIR COMPLETED."
2885   REM end of damage-progress loop
     NEXT I
     IF RND(1) > 0.2 THEN GOTO 3070
2910 LET R1 = FNR(1)
     IF RND(1) >= 0.6 THEN GOTO 3000
...
3240   LET S8 = INT(S1)*24 + INT(S2)*3 - 26
       IF MID$(Q$, S8, 2) = "  " THEN GOTO 3365
3320   LET S1 = INT(S1 - X1)
       LET S2 = INT(S2 - X2)
       PRINT "WARP ENGINES SHUT DOWN AT ";
3350   PRINT "SECTOR"; S1; ","; S2; "DUE TO BAD NAVIGATION"
       GOTO 3370
3365   REM continue warp-step loop
     NEXT I
3360 LET S1 = INT(S1)
     LET S2 = INT(S2)
3370 LET A$ = "<*>"
     ...
     GOSUB 3910
     LET T8 = 1
3430 IF W1 < 1 THEN LET T8 = 0.1 * INT(10*W1)
3450 LET T = T + T8
...
3550 LET S2 = INT(Y - Q2*8)
     IF S1 = 0 THEN LET Q1 = Q1 - 1 : LET S1 = 8
3590 IF S2 = 0 THEN LET Q2 = Q2 - 1 : LET S2 = 8
3620 LET X5 = 0
     IF Q1 < 1 THEN LET X5 = 1 : LET Q1 = 1 : LET S1 = 1
...
4060 FOR I = Q1-1 TO Q1+1
       ...
       LET NB(3) = -3
       FOR J = Q2-1 TO Q2+1
4120     IF I > 0 AND I < 9 AND J > 0 AND J < 9 THEN LET NB(J-Q2+2) = G(I,J) : LET Z(I,J) = G(I,J)
       NEXT J
       FOR L = 1 TO 3
         PRINT ": ";
         IF NB(L) < 0 THEN PRINT "*** "; : GOTO 4235
4210     PRINT RIGHT$(STR$(NB(L)+1000), 3); " ";
4235     REM continue LRS row
       NEXT L
       PRINT ":"
       PRINT O1$
     NEXT I
...
4370 INPUT "NUMBER OF UNITS TO FIRE"; X
     IF X <= 0 THEN GOTO 1990
4400 IF E - X < 0 THEN GOTO 4360
4410 LET E = E - X
     IF D(7) < 0 THEN LET X = X * RND(1)
4450 LET H1 = INT(X / K3)
     FOR I = 1 TO 3
       IF K(I,3) <= 0 THEN GOTO 4675
4480   LET H = INT((H1 / FND(0)) * (RND(1) + 2))
       IF H > 0.15 * K(I,3) THEN GOTO 4530
...
4580   LET K3 = K3 - 1
       ...
       GOSUB 8670
4650   LET K(I,3) = 0
       LET G(Q1,Q2) = G(Q1,Q2) - 100
       LET Z(Q1,Q2) = G(Q1,Q2)
       IF K9 <= 0 THEN GOTO 6370
4675   REM continue phaser target loop
     NEXT I
     GOSUB 6000
     GOTO 1990
...
5060 LET A$ = "+K+"
     ...
     LET Z2 = Y
     GOSUB 8830
     IF Z3 = 0 THEN GOTO 5210
5110 PRINT "*** KLINGON DESTROYED ***"
     LET K3 = K3 - 1
     LET K9 = K9 - 1
     IF K9 <= 0 THEN GOTO 6370
5150 FOR I = 1 TO 3
       IF X3 = K(I,1) AND Y3 = K(I,2) THEN GOTO 5195
5180 NEXT I
     LET I = 3
5195 LET K(I,3) = 0
     GOTO 5430
...
5700 PRINT "DAMAGE CONTROL REPORT NOT AVAILABLE"
     IF D0 = 0 THEN GOTO 1990
5720 LET D3 = 0
     FOR I = 1 TO 8
       IF D(I) < 0 THEN LET D3 = D3 + 0.1
     NEXT I
     IF D3 = 0 THEN GOTO 1990
5780 PRINT
     LET D3 = D3 + D4
     IF D3 >= 1 THEN LET D3 = 0.9
...
6430 FOR I = S1-1 TO S1+1
       FOR J = S2-1 TO S2+1
6450     IF INT(I+0.5) < 1 OR INT(I+0.5) > 8 OR INT(J+0.5) < 1 OR INT(J+0.5) > 8 THEN GOTO 6545
6490     LET A$ = ">!<"
         LET Z1 = I
         LET Z2 = J
         GOSUB 8830
         IF Z3 = 1 THEN GOTO 6580
6545     REM continue starbase-adjacency scan
       NEXT J
     NEXT I
     LET D0 = 0
     GOTO 6650
...
6620 PRINT "SHIELDS DROPPED FOR DOCKING PURPOSES"
     LET S = 0
     GOTO 6720
6650 IF K3 > 0 THEN LET C$ = "*RED*" : GOTO 6720
6660 LET C$ = "GREEN"
     IF E < E0*0.1 THEN LET C$ = "YELLOW"
...
7570 PRINT O1$
     FOR I = 1 TO 8
       PRINT I;
       IF H8 = 0 THEN GOTO 7740
7630   FOR J = 1 TO 8
         PRINT "   ";
         IF Z(I,J) = 0 THEN PRINT "***"; : GOTO 7725
7700     PRINT RIGHT$(STR$(Z(I,J)+1000), 3);
7725     REM continue galactic-record row
       NEXT J
       GOTO 7850
7740   LET Z4 = I
       LET Z5 = 1
       GOSUB 9030
       LET J0 = INT(15 - 0.5*LEN(G2$))
       PRINT TAB(J0); G2$;
7800   LET Z5 = 5
       GOSUB 9030
       LET J0 = INT(39 - 0.5*LEN(G2$))
       PRINT TAB(J0); G2$;
7850   PRINT
       PRINT O1$
     NEXT I
...
8080 LET X$ = ""
     IF K3 > 1 THEN LET X$ = "S"
8090 PRINT "FROM ENTERPRISE TO KLINGON BATTLE CRUISER"; X$
8100 LET H8 = 0
     FOR I = 1 TO 3
       IF K(I,3) <= 0 THEN GOTO 8485
8110   LET W1 = K(I,1)
       LET X = K(I,2)
8120   LET C1 = S1
       LET A = S2
       GOSUB 8210
8485   REM end of klingon-nav loop
     NEXT I
     GOTO 1990
...
8200 INPUT "  FINAL COORDINATES (X,Y)"; W1, X
     GOSUB 8210
     GOTO 1990
8210 REM D/D COMPUTATION SUBROUTINE (formerly inlined at 8220)
8220 LET X = X - A
     LET A = C1 - W1
     IF X < 0 THEN GOTO 8350
8250 IF A < 0 THEN GOTO 8410
8260 IF X > 0 THEN GOTO 8280
8270 IF A = 0 THEN LET C1 = 5 : GOTO 8290
8275 GOTO 8280
8280 LET C1 = 1
...
8330 PRINT "DIRECTION ="; C1 + (ABS(A) / ABS(X))
     GOTO 8460
8350 IF A > 0 THEN LET C1 = 3 : GOTO 8420
8360 IF X <> 0 THEN LET C1 = 5 : GOTO 8290
8410 LET C1 = 7
...
8670 LET S8 = INT(Z2 - 0.5)*3 + INT(Z1 - 0.5)*24 + 1
8675 IF LEN(A$) <> 3 THEN PRINT "ERROR" : STOP
8680 IF S8 = 1 THEN LET Q$ = A$ & RIGHT$(Q$, 189) : RETURN
8690 IF S8 = 190 THEN LET Q$ = LEFT$(Q$, 189) & A$ : RETURN
8700 LET Q$ = LEFT$(Q$, S8 - 1) & A$ & RIGHT$(Q$, 190 - S8)
     RETURN
8780 REM PRINTS DEVICE NAME
8790 IF R1 = 1 THEN LET G2$ = "WARP ENGINES" : RETURN
8794 IF R1 = 2 THEN LET G2$ = "SHORT RANGE SENSORS" : RETURN
8796 IF R1 = 3 THEN LET G2$ = "LONG RANGE SENSORS" : RETURN
8798 IF R1 = 4 THEN LET G2$ = "PHASER CONTROL" : RETURN
8800 IF R1 = 5 THEN LET G2$ = "PHOTON TUBES" : RETURN
8802 IF R1 = 6 THEN LET G2$ = "DAMAGE CONTROL" : RETURN
8804 IF R1 = 7 THEN LET G2$ = "SHIELD CONTROL" : RETURN
8806 LET G2$ = "LIBRARY-COMPUTER"
     RETURN
8820 REM STRING COMPARISON IN QUADRANT ARRAY
8830 LET Z1 = INT(Z1 + 0.5)
     LET Z2 = INT(Z2 + 0.5)
     LET S8 = (Z2 - 1)*3 + (Z1 - 1)*24 + 1
     LET Z3 = 0
...
9032 IF Z4 = 1 THEN LET G2$ = "ANTARES" : GOTO 9210
     IF Z4 = 2 THEN LET G2$ = "RIGEL" : GOTO 9210
     IF Z4 = 3 THEN LET G2$ = "PROCYON" : GOTO 9210
     IF Z4 = 4 THEN LET G2$ = "VEGA" : GOTO 9210
     IF Z4 = 5 THEN LET G2$ = "CANOPUS" : GOTO 9210
     IF Z4 = 6 THEN LET G2$ = "ALTAIR" : GOTO 9210
     IF Z4 = 7 THEN LET G2$ = "SAGITTARIUS" : GOTO 9210
     IF Z4 = 8 THEN LET G2$ = "POLLUX" : GOTO 9210
9120 IF Z4 = 1 THEN LET G2$ = "SIRIUS" : GOTO 9210
     IF Z4 = 2 THEN LET G2$ = "DENEB" : GOTO 9210
     IF Z4 = 3 THEN LET G2$ = "CAPELLA" : GOTO 9210
     IF Z4 = 4 THEN LET G2$ = "BETELGEUSE" : GOTO 9210
     IF Z4 = 5 THEN LET G2$ = "ALDEBARAN" : GOTO 9210
     IF Z4 = 6 THEN LET G2$ = "REGULUS" : GOTO 9210
     IF Z4 = 7 THEN LET G2$ = "ARCTURUS" : GOTO 9210
     IF Z4 = 8 THEN LET G2$ = "SPICA"
9210 IF G5 = 1 THEN RETURN
     IF Z5 = 1 OR Z5 = 5 THEN LET G2$ = G2$ & " I" : RETURN
     IF Z5 = 2 OR Z5 = 6 THEN LET G2$ = G2$ & " II" : RETURN
     IF Z5 = 3 OR Z5 = 7 THEN LET G2$ = G2$ & " III" : RETURN
     IF Z5 = 4 OR Z5 = 8 THEN LET G2$ = G2$ & " IV" : RETURN
     RETURN
