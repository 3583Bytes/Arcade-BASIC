...
| **Audio** (`SOUND`/`BEEP`/`PLAY`) | A `BasicAudioOutput` component (added at runtime) streams the program's audio through a looping `AudioClip`: the engine-agnostic `BufferedAudioDevice` renders tones to PCM on the BASIC thread, and the clip's PCM-reader callback drains it on Unity's audio thread. A fresh device per Run; Stop/finish silences it. Foreground (`MF`) and background (`MB`) music both work. |
